...
class MLP(nn.Module):
    def __init__(self, params):
        ...
        self.in_channels = params.inp_dim
        self.hidden_channels = params.hidden_dim
        self.out_channels = params.out_dim
...
class GCN(nn.Module):
    def __init__(self, params, save_mem=True, use_bn=True):
...
class GAT(nn.Module):
    def __init__(self, params, use_bn=False, heads=2, out_heads=1):
        ...
        self.convs = nn.ModuleList()
        self.convs.append(
            GATConv(self.in_channels, self.hidden_channels, dropout=self.dropout, heads=heads, concat=True))

        self.bns = nn.ModuleList()
        self.bns.append(nn.BatchNorm1d(self.hidden_channels*heads))
        for _ in range(self.num_layers - 2):
            self.convs.append(
                    GATConv(self.hidden_channels*heads, self.hidden_channels, dropout=self.dropout, heads=heads, concat=True))
            self.bns.append(nn.BatchNorm1d(self.hidden_channels*heads))

        self.convs.append(
            GATConv(self.hidden_channels*heads, self.out_channels, dropout=self.dropout, heads=out_heads, concat=False))
...
class MixHop(nn.Module):
    ...
    def __init__(self, params):
        ...
        self.hidden_channels = params.hidden_dim
        self.out_channels = params.out_dim
        self.hops = params.hop
        self.num_layers = params.num_layers
        self.convs = nn.ModuleList()
        self.convs.append(MixHopLayer(self.in_channels, self.hidden_channels, hops=self.hops))

        self.bns = nn.ModuleList()
        self.bns.append(nn.BatchNorm1d(self.hidden_channels*(self.hops+1)))
        for _ in range(self.num_layers - 2):
            self.convs.append(
                MixHopLayer(self.hidden_channels*(self.hops+1), self.hidden_channels, hops=self.hops))
            self.bns.append(nn.BatchNorm1d(self.hidden_channels*(self.hops+1)))

        self.convs.append(
            MixHopLayer(self.hidden_channels*(self.hops+1), self.out_channels, hops=self.hops))

        # note: uses linear projection instead of paper's attention output
        self.final_project = nn.Linear(self.out_channels*(self.hops+1), self.out_channels)
...
class GCNJK(nn.Module):
    def __init__(self, params, save_mem=False, jk_type='max'):
        ...
        self.convs = nn.ModuleList()
        self.convs.append(
            GCNConv(self.in_channels, self.hidden_channels, cached=not save_mem, normalize=not save_mem))

        self.bns = nn.ModuleList()
        self.bns.append(nn.BatchNorm1d(self.hidden_channels))
        for _ in range(self.num_layers - 2):
            self.convs.append(
                GCNConv(self.hidden_channels, self.hidden_channels, cached=not save_mem, normalize=not save_mem))
            self.bns.append(nn.BatchNorm1d(self.hidden_channels))
        ...
        self.activation = F.relu
        self.jump = JumpingKnowledge(jk_type, channels=self.hidden_channels, num_layers=1)
        if jk_type == 'cat':
            self.final_project = nn.Linear(self.hidden_channels * self.num_layers, self.out_channels)
        else: # max or lstm
            self.final_project = nn.Linear(self.hidden_channels, self.out_channels)
...
class H2GCN(nn.Module):
    """ our implementation """
    def __init__(self, params, edge_index, save_mem=False, num_mlp_layers=1,
        ...
        self.feature_embed = MLP(self.in_channels, self.hidden_channels,
                self.hidden_channels, num_layers=num_mlp_layers, dropout=self.dropout)
        ...
        self.bns = nn.ModuleList()
        self.bns.append(nn.BatchNorm1d(self.hidden_channels*2*len(self.convs) ) )
        ...
        last_dim = self.hidden_channels*(2**(self.num_layers+1)-1)
        self.final_project = nn.Linear(last_dim, self.out_channels)
